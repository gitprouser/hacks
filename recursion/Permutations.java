...
public class Permutations {
    ...
    private static void helper(List<List<Integer>> result, Stack<Integer> stack, int[] nums, int indent) {
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            for(int j =0; j < indent; j++)
                System.out.print("\t");
            System.out.println("------->i = " + i + ", after push, before helper: " + stack);
            helper(result, stack, nums, ++indent);
            // go back up one level, and in the next iteration, visit the next element
            // (with index i++ in nums[]) in the same level
            for(int j =0; j < indent; j++)
                System.out.print("\t");
            System.out.println("------->i = " + i + ", after helper, before pop: " + stack);
            stack.pop();
            for(int j =0; j < indent; j++)
                System.out.print("\t");
