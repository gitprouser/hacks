 ...
 *
 ...
 (This problem is not exactly RLE, but it's very similar. Basic RLE is actually a trivial problem; but this one here is slightly more complicated)



 L10n, is called a Numeronym of the word Localization, where 10 stands for the number of letters between the first 'L' and the last 'n' in the word.
    ...
    public static void main(String[] args) {
        ...
        for (int i = s.length() - 2; i > 1; i--) {
            for ( int j = 1; j + i < s.length(); j++) {
                System.out.println(s.substring(0, j) + i + s.substring(j + i, s.length()));
            }
        }
    }
}
